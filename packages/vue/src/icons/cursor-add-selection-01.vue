...
defineOptions({ inheritAttrs: false, name: 'CursorAddSelection01Icon' })
...
const plusVariants: Variants = {
  ...
  animate: {
    transform: ['rotate(0deg) scale(1)', 'rotate(-9.64deg) scale(0.82)', 'rotate(2.89deg) scale(1.08)', 'rotate(0deg) scale(1)'],
    transition: {
      duration: 0.4,
      delay: 0.04,
      ease: [0.23, 1, 0.32, 1],
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M15 3V9M18 6L12 6" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="plusVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '15px 6px' }" />
          </svg>
        </div>
</template>
