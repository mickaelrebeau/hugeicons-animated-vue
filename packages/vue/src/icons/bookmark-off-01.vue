...
defineOptions({ inheritAttrs: false, name: 'BookmarkOff01Icon' })
...
const markVariants: Variants = {
  ...
  animate: {
    transform: ['scale(1) rotate(0deg)', 'scale(0.86) rotate(-1.88deg)', 'scale(1.08) rotate(0.75deg)', 'scale(1) rotate(0deg)'],
    transition: {
      duration: 0.49,
      ease: [0.23, 1, 0.32, 1],
    },
...
const iconVariants: Variants = {
  ...
  animate: {
    transform: ['rotate(0deg) translateY(0px)', 'rotate(-8.26deg) translateY(0.41px)', 'rotate(0deg) translateY(1.24px)', 'rotate(0deg) translateY(0px)'],
    opacity: [1, 1, 0.55, 1],
    transition: {
      duration: 0.71,
      ease: [0.77, 0, 0.175, 1],
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M11 7L19.5 7" stroke="currentColor" stroke-linecap="round" stroke-width="1.5" :variants="iconVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '15.25px 7px' }" />
