...
defineOptions({ inheritAttrs: false, name: 'ChatCancelIcon' })
...
const markVariants: Variants = {
  ...
  animate: {
    transform: ['scale(1) rotate(0deg)', 'scale(0.86) rotate(-1.87deg)', 'scale(1.08) rotate(0.75deg)', 'scale(1) rotate(0deg)'],
    transition: {
      duration: 0.56,
      ease: [0.23, 1, 0.32, 1],
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M15 13.5L9 7.5M15 7.5L9 13.5" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="markVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 10.5px' }" />
          </svg>
        </div>
</template>
