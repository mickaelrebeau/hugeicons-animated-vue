...
defineOptions({ inheritAttrs: false, name: 'SevenZIcon' })
...
// after arrow-right-02: the seven z follows the right direction already encoded by its geometry
...
const arrowVariants: Variants = {
  ...
  animate: {
    transform: ['translateX(0px) scaleY(1)', 'translateX(2.43px) scaleY(0.94)', 'translateX(-0.28px) scaleY(1.02)', 'translateX(0.42px) scaleY(0.99)', 'translateX(0px) scaleY(1)'],
    transition: {
      duration: 0.53,
      ease: [0.23, 1, 0.32, 1],
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M6 16H10L7 22M14 16H18L14 22H18" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="arrowVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 12px' }" />
          </svg>
        </div>
</template>
