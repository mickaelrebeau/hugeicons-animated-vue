...
const iconVariants: Variants = {
  ...
  animate: {
    transform: ['rotate(0deg)', 'rotate(-10.09deg)', 'rotate(8.07deg)', 'rotate(-5.04deg)', 'rotate(0deg)'],
    transition: {
      duration: 0.69,
      ease: 'easeInOut',
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M14 4V10M18 4V10" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="iconVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '11.94px 3px' }" />
          </svg>
        </div>
</template>
