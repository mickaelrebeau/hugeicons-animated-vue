...
defineOptions({ inheritAttrs: false, name: 'CallPausedIcon' })
...
// the pause bars blink while the handset waits
// authored from scripts/authored
const handsetVariants: Variants = {
  normal: { transform: 'translateY(0px)' },
  animate: {
    transform: ['translateY(0px)', 'translateY(0.8px)', 'translateY(0px)'],
    transition: { duration: 0.8, times: [0, 0.5, 1], ease: [0.77, 0, 0.175, 1] },
  },
};
...
const pauseVariants: Variants = {
  ...
  animate: {
    ...
    transition: { duration: 0.74, times: [0, 0.18, 0.4, 0.62, 1], ease: [0.23, 1, 0.32, 1] },
  },
};
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M14 4V10M18 4V10" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="pauseVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '16px 7px' }" />
          </svg>
        </div>
</template>
