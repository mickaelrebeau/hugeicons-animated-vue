...
defineOptions({ inheritAttrs: false, name: 'BookmarkIcon' })
...
const iconVariants: Variants = {
  ...
  animate: {
    transform: ['translateY(0px) rotate(0deg)', 'translateY(-1.39px) rotate(-2.97deg)', 'translateY(0.3px) rotate(0.79deg)', 'translateY(0px) rotate(0deg)'],
    transition: {
      duration: 0.55,
      ease: [0.23, 1, 0.32, 1],
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M4 7H20" stroke="currentColor" stroke-width="1.5" :variants="iconVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 2px' }" />
          </svg>
        </div>
</template>
