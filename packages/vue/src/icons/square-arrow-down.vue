...
defineOptions({ inheritAttrs: false, name: 'SquareArrowDownIcon' })
...
const arrowVariants: Variants = {
  ...
  animate: {
    transform: ['translateY(0px) scaleX(1)', 'translateY(2.53px) scaleX(0.94)', 'translateY(-0.29px) scaleX(1.02)', 'translateY(0.44px) scaleX(0.99)', 'translateY(0px) scaleX(1)'],
    transition: {
      duration: 0.48,
      ease: [0.23, 1, 0.32, 1],
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M17 10C17 10 13.3176 14 12 14C10.6824 14 7 10 7 10" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="arrowVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 12px' }" />
          </svg>
        </div>
</template>
