...
const iconVariants: Variants = {
  ...
  animate: {
    transform: ['rotate(0deg)', 'rotate(-9.22deg)', 'rotate(7.38deg)', 'rotate(-4.61deg)', 'rotate(0deg)'],
    transition: {
      duration: 0.74,
      ease: 'easeInOut',
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M20 4L14 10M20 10L14 4" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="iconVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '11.94px 3px' }" />
          </svg>
        </div>
</template>
