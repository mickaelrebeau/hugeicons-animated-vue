...
defineOptions({ inheritAttrs: false, name: 'CreditCardNotAcceptIcon' })
...
const iconVariants: Variants = {
  ...
  animate: {
    transform: ['translateX(0px) rotate(0deg)', 'translateX(2.34px) rotate(1.95deg)', 'translateX(-0.29px) rotate(-0.49deg)', 'translateX(0px) rotate(0deg)'],
    transition: {
      duration: 0.54,
      ease: [0.23, 1, 0.32, 1],
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M22 14L16 20M22 20L16 14" stroke="currentColor" stroke-linecap="round" stroke-width="1.5" :variants="iconVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 12px' }" />
          </svg>
        </div>
</template>
