...
defineOptions({ inheritAttrs: false, name: 'UserBlock01Icon' })
...
const headVariants: Variants = {
  ...
  animate: {
    transform: ['rotate(0deg)', 'rotate(16.21deg)', 'rotate(-4.05deg)', 'rotate(0deg)'],
    transition: {
      duration: 0.6,
      ease: 'easeInOut',
      times: [0, 0.4, 0.7, 1],
...
const personVariants: Variants = {
  ...
  animate: {
    transform: ['translateY(0px) scale(1)', 'translateY(-1.39px) scale(1.03)', 'translateY(0.46px) scale(0.98)', 'translateY(0px) scale(1)'],
    transition: {
      duration: 0.59,
      ease: [0.23, 1, 0.32, 1],
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M15.55 15.05L20.45 19.95M21.5 17.5C21.5 15.567 19.933 14 18 14C16.067 14 14.5 15.567 14.5 17.5C14.5 19.433 16.067 21 18 21C19.933 21 21.5 19.433 21.5 17.5Z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="personVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '18px 17.5px' }" />
          </svg>
        </div>
</template>
