<script setup lang="ts">
import { motion, useAnimationControls } from 'motion-v'
import type { Variants } from 'motion-v'
import { useIconAnimation } from '../composables/useIconAnimation'
import type { AnimatedIconHandle } from '../types'

defineOptions({ inheritAttrs: false, name: 'ArtificialIntelligence05Icon' })

withDefaults(
  defineProps<{
    size?: number
  }>(),
  { size: 28 },
)

// the gear plate ticks one tooth, then the letters ride the face
// authored from scripts/authored
const plateVariants: Variants = {
  normal: { transform: 'rotate(0deg)' },
  animate: {
    transform: ['rotate(0deg)', 'rotate(-8deg)', 'rotate(22deg)', 'rotate(-3deg)', 'rotate(0deg)'],
    transition: {
      duration: 0.76,
      times: [0, 0.18, 0.46, 0.74, 1],
      ease: [
        [0.77, 0, 0.175, 1],
        [0.23, 1, 0.32, 1],
        [0.23, 1, 0.32, 1],
        [0.23, 1, 0.32, 1],
      ],
    },
  },
};

const chipVariants: Variants = {
  normal: { transform: 'scale(1)', opacity: 1 },
  animate: {
    transform: ['scale(1)', 'scale(0.88)', 'scale(1.07)', 'scale(1)'],
    opacity: [1, 0.62, 1, 1],
    transition: { duration: 0.56, delay: 0.14, times: [0, 0.3, 0.66, 1], ease: [0.23, 1, 0.32, 1] },
  },
};

const eyeIVariants: Variants = {
  normal: { transform: 'scaleY(1)' },
  animate: {
    transform: ['scaleY(1)', 'scaleY(0.22)', 'scaleY(1.08)', 'scaleY(1)'],
    transition: {
      duration: 0.5,
      delay: 0.22,
      times: [0, 0.14, 0.62, 1],
      ease: [
        [0.77, 0, 0.175, 1],
        [0.23, 1, 0.32, 1],
        [0.23, 1, 0.32, 1],
      ],
    },
  },
};

const controls = useAnimationControls()
const { onMouseEnter, onMouseLeave, startAnimation, stopAnimation } = useIconAnimation({
  controls,
  loops: false,
})

defineExpose<AnimatedIconHandle>({ startAnimation, stopAnimation })
</script>

<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            <motion.path d="M21.255 7.13359L20.7614 6.27699C20.3881 5.62916 20.2015 5.30525 19.8839 5.17608C19.1739 4.88734 17.9756 5.49725 17.2686 5.69638C16.6419 5.84091 16.1039 5.63881 15.5732 5.33262C15.2141 5.10263 14.9379 4.76353 14.785 4.36494C14.5708 3.72506 14.427 2.59797 13.8603 2.18877C13.599 2.00001 13.2518 2.00001 12.5574 2.00001H11.4426C10.7483 2.00001 10.4011 2.00001 10.1397 2.18877C9.57309 2.598 9.42928 3.72508 9.21503 4.36494C9.00249 4.91906 8.58201 5.24311 8.08999 5.52699C7.67124 5.74214 7.19009 5.80214 6.73148 5.69638C6.02454 5.49725 4.82615 4.88733 4.11617 5.17608C3.79859 5.30525 3.61195 5.62916 3.23865 6.27699L2.74507 7.13359C2.39516 7.74084 2.2202 8.04447 2.25416 8.36769C2.33849 9.1706 3.50758 10.0501 4.0218 10.625C4.2738 10.944 4.45271 11.5 4.45271 11.9999C4.45271 12.5 4.27386 13.0558 4.02183 13.3749C3.50757 13.9498 2.3385 14.8293 2.25416 15.6323C2.2202 15.9555 2.39516 16.2591 2.74507 16.8663L3.23864 17.7229C3.61193 18.3707 3.79859 18.6947 4.11617 18.8238C4.8261 19.1126 6.02453 18.5026 6.73144 18.3035C7.71336 18.077 8.85351 18.6924 9.215 19.635C9.42926 20.2749 9.57308 21.402 10.1397 21.8113C10.4011 22 10.7483 22 11.4426 22H12.5574C13.2518 22 13.599 22 13.8603 21.8113C14.1217 21.6225 14.2315 21.2925 14.4511 20.6325C14.6985 19.8935 14.8719 19.1165 15.5731 18.6674C16.1038 18.3611 16.6418 18.1589 17.2686 18.3035C17.9755 18.5026 19.1739 19.1126 19.8839 18.8238C20.2015 18.6947 20.3881 18.3707 20.7614 17.7229L21.255 16.8663C21.6049 16.2591 21.7798 15.9555 21.7459 15.6323C21.6614 14.8293 20.4925 13.9499 19.9782 13.3749C19.3955 12.6371 19.3956 11.3628 19.9782 10.625C20.4925 10.0501 21.6614 9.17068 21.7459 8.36769C21.7798 8.04447 21.6049 7.74084 21.255 7.13359Z" stroke="currentColor" stroke-linecap="round" stroke-width="1.5" :variants="plateVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 12px' }" />
            <motion.path d="M7.4375 14.9922L9.27939 9.46653C9.37381 9.18326 9.6389 8.99219 9.9375 8.99219C10.2361 8.99219 10.5012 9.18326 10.5956 9.46653L12.4375 14.9922M8.4375 12.9922H11.4375" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="chipVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '9.9px 12px' }" />
            <motion.path d="M15.4375 8.99219V14.9922" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="eyeIVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '15.4px 15px' }" />
          </svg>
        </div>
</template>
