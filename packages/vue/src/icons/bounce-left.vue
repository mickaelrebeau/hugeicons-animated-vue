...
const arrowVariants: Variants = {
  ...
  animate: {
    transform: ['translateX(0px) scaleY(1)', 'translateX(-2.81px) scaleY(0.94)', 'translateX(0.32px) scaleY(1.02)', 'translateX(-0.49px) scaleY(0.99)', 'translateX(0px) scaleY(1)'],
    transition: {
      duration: 0.52,
      ease: [0.23, 1, 0.32, 1],
    },
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.circle cx="2" cy="2" r="2" transform="matrix(-1 0 0 1 8 3)" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="arrowVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 12px' }" />
          </svg>
        </div>
</template>
