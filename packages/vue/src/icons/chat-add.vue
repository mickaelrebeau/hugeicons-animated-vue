...
defineOptions({ inheritAttrs: false, name: 'ChatAddIcon' })
...
const plusVariants: Variants = {
  ...
  animate: {
    transform: ['rotate(0deg) scale(1)', 'rotate(-10.61deg) scale(0.82)', 'rotate(3.18deg) scale(1.08)', 'rotate(0deg) scale(1)'],
    transition: {
      duration: 0.43,
      delay: 0.04,
      ease: [0.23, 1, 0.32, 1],
...
<template>
  <div class="hia-icon" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-bind="$attrs">
          <svg xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" viewBox="0 0 24 24" fill="none" overflow="visible">
            ...
            <motion.path d="M15.5 10.5H8.5M12 7V14" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :variants="plusVariants" :animate="controls" initial="normal" :style="{ transformOrigin: '12px 10.5px' }" />
          </svg>
        </div>
</template>
